...
#include <string.h>
#include "TH02_dev.h"
#include <SD.h> /* AB library for SD card */
...
char    ssid[48]    = /*"DG860AB2";*/"NETGEAR22222";       /* your network SSID (name) */
char    psw[48]     = /*"DG860AAFFCB2";*/"Junqi987654321";     /* your network password */
...
void RelayHandle( int );


void BuzzerHandle( int );


void ServoHandle( int );


void SleepHandle( int );
...
Servo   myservo;
File    myFile; /* AB an object for the the text file were are using to save data. */
...
void SerialRequestHandler()
{
  if ( cmdstrInput( cmdstr ) )
  {
    ...
    myFile.println( cmdstr );
    if ( !parsecmd( cmdstr ) )
    {
      Serial.println( "  ---- FAIL!" );
      myFile.println( "  ---- FAIL!" );
      Serial.println( "Please enter command: set temp>50,relay=1 etc." );
      myFile.println( "Please enter command: set temp>50,relay=1 etc." );
    }else  {
      Serial.println( "  ---- OK!" );
      myFile.println( "  ---- OK!" );
    }
 ...
 */
  ...
  {
    if ( *cp1 != ' ' )
      tmpcmd[len++] = *cp1;   /* delete space */
    cp1++;
  }
  tmpcmd[len] = '\0';
  if ( len < 7 )
    return(0);                      /* at least 7 characters,like T>0,R=1 */
  ...
  if ( i == VAR2_COUNT )
    return(0);      /* Illegal actuator name */

  if ( op1 == '\0' || op2 == '\0' )
    return(0);      /* Illegal operator name */
...
void setup()
{
  ...
  /* AB Then check if a previous text file exsits */
  if ( SD.exists( "savedDataHW1.txt" ) )
  {
    Serial.println( "savedDataHW1.txt exists! removing that and creating another one." );
    /* SD.remove("savedDataHW1.txt"); */
  }else Serial.println( "savedDataHW1.txt doesn't exist, creating a new one..." );
  /* AB now open or create the file */
  myFile = SD.open( "savedDataHW1.txt", FILE_WRITE );
  myFile.println( "START, AB and CB HW1 DATA..." );
  /*
   * myFile.close();
   * myFile = SD.open("savedDataHW1.txt", FILE_WRITE);
   */
...
void WiFi_Init()
{
  ...
  if ( WiFi.status() == WL_NO_SHIELD )
  {
    ...
    myFile.println( "WiFi shield not present" );
    /* don't continue: */
    while ( true )
      ;
  }
  ...
  myFile.println( "Please upgrade the firmware" );
}
...
void RequestHandler()
{
  ...
  if ( status != WL_CONNECTED && *ssid != '\0' && *psw != '\0' )
  {
    if ( (millis() - cur) > 5000 )
    {
      ...
      myFile.print( "Attempting to connect to SSID: " );
      Serial.println( ssid );
      myFile.println( ssid );
      /* Connect to WPA/WPA2 network. Change this line if using open or WEP network: */
      status  = WiFi.begin( ssid, psw );
      Serial.print( "WIFI return status: " );
      myFile.print( "WIFI return status: " );
...
void sendpage()
{
  ...
  if ( client )
  {
    ...
    myFile.println( "client disonnected" );
  }
}
